...
처음 만든 산점도는 정확도 범위가 98.40~98.54%로 너무 좁아, 점의 위치보다 실험 이름만 눈에 띄었다. 그래서 전체 데이터 실험은 기준 실험 대비 Test 정확도가 얼마나 올랐는지, 그 대가로 학습 시간과 파라미터 수가 얼마나 늘었는지를 나란히 비교했다.

![Full Data Effect Summary](report_assets/diagnostics/full_data_effect_summary.png)

왼쪽 그래프는 기준 대비 정확도 증가량이고, 오른쪽 그래프는 기준 대비 학습 시간과 파라미터 수의 배율이다. 가장 큰 정확도 증가는 `batch_64`와 `wide_1024_512`의 +0.14%p였다. 하지만 `batch_64`는 학습 시간이 1.67배로 늘었고, `wide_1024_512`는 학습 시간이 2.72배, 파라미터 수가 2.49배로 늘었다. 반대로 `epochs_30`은 반복 횟수와 업데이트 수가 증가했지만 Test 정확도는 기준과 같았다. 즉 전체 데이터 기준에서는 성능이 이미 포화되어, 정확도 차이보다 비용 차이가 더 뚜렷하게 나타났다.
